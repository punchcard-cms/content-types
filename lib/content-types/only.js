  ...
  return new Promise((resolve) => {
    ...
    const attrs = mergedType.attributes.map(attribute => {
      ...
      if (Array.isArray(config[attr.id])) {
        const instance = [];
        config[attr.id].forEach((data, index) => {
          instance[index] = merge(attr.inputs, data);
          inputs.forEach(input => {
            instance[index][input].validation = attribute.inputs[input].validation;
            instance[index][input].type = attribute.inputs[input].type;
            instance[index][input].id = `${attribute.inputs[input].id}--${index}`;
            instance[index][input].name = `${attribute.inputs[input].name}--${index}`;
          });
        });
        attr.inputs = instance;
      }
      else {
