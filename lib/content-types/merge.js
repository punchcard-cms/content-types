  ...
  return new Promise((resolve, reject) => {
    ...
    const configured = types.map(type => {
      ...
      if (ids.indexOf(ctID) < 0) {
        reject(`Identifier '${ctID}' was not found in content type '${type.id}'`);
      }
      ...
      if (foundID.hasOwnProperty('repeatable')) {
        reject(`Identifier '${ctID}' in content type '${type.id}' must not be a repeatable attribute`);
      }

      // No multi-input inputs
      if (Object.keys(foundID.inputs).length > 1) {
        reject(`Identifier '${ctID}' in content type '${type.id}' must be a single input attribute`);
      }

      // No inputs with multiple
      if (foundInput.hasOwnProperty('options')) {
        reject(`Identifier '${ctID}' in content type '${type.id}' must not have options`);
      }
