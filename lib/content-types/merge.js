    ...
    const configured = types.map(type => {
      ...
      const attrs = mergedType.attributes.map(attribute => {
        ...
        if (attribute.hasOwnProperty('repeatable')) {
          // Checks if repeatable is an object or boolean
          if (typeof attribute.repeatable === 'object') {
            // Blank object to remove extraneous keys from object
            plugin.repeatable = {};

            // Check and sets default value of min
            ...
            // Check and sets default value of max
            ...
            // sets default value of min and max
            plugin.repeatable = {
              min: 1,
        ...
        if (attribute.hasOwnProperty('inputs')) {
