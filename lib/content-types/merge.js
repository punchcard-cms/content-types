...
const squish = (types) => {
  return new Promise((resolve, reject) => {
    ...
    const configured = types.map(type => {
      ...
      const ctID = type.identifier;
      const ids = [];

      if (!type.hasOwnProperty('identifier')) {
        reject(new Error(`Content type '${type.id}' requires an identifier`));
      }
      ...
      // Ensure Identifier
      if (ids.indexOf(ctID) < 0) {
        reject(new Error(`Identifier '${ctID}' was not found in content type '${type.id}'`));
      }

      // Get the actual identifier input
      const foundID = util.singleItem('id', ctID, attrs);
      const foundInput = foundID.inputs[Object.keys(foundID.inputs)[0]];

      // Weed out the bad types of inputs
      // No Repeatables
      if (foundID.hasOwnProperty('repeatable')) {
        reject(new Error(`Identifier '${ctID}' in content type '${type.id}' must not be a repeatable attribute`));
      }

      // No multi-input inputs
      if (Object.keys(foundID.inputs).length > 1) {
        reject(new Error(`Identifier '${ctID}' in content type '${type.id}' must be a single input attribute`));
      }

      // No inputs with multiple
      if (foundInput.hasOwnProperty('options')) {
        reject(new Error(`Identifier '${ctID}' in content type '${type.id}' must not have options`));
      }
