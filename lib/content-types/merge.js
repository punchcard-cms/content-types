...
const squish = (types) => {
  return new Promise((resolve, reject) => {
    ...
    const configured = types.map(type => {
      ...
      const attrs = mergedType.attributes.map((atr) => {
        const attribute = atr;
        ...
        // we need to know the name here in case there is an id issue
        if (attribute.name) {
        ...
        if (attribute.hasOwnProperty('repeatable')) {
          // check if repeatable object is in config
          ...
          else if (attribute.hasOwnProperty('repeatable') && attribute.repeatable === true) {
            // if repeatable is true, we make the object for them
            plugin.repeatable = {
              min: 1,
