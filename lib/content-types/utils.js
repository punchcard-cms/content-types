...
const attributes = (config) => {
  ...
  const attrs = config.attributes.map(attr => {
    if ((!attr.hasOwnProperty('name') || attr.name === '')) {
      let ident = attr.type;
      ...
      return `Attribute '${ident}' in content type '${config.name}' needs a name`;
    }
    if (typeof attr.name !== 'string') {
      return `Attribute names in content type '${config.name}' must be a string`;
    }
    if (!attr.id || attr.id === '') {
      return `Attribute '${attr.name}' in content type '${config.name}' must have an id`;
    }
