 ...
 *
 * @param {object|array} ctObj content type object
 */
const merged = (ctObj) => {
  if (ctObj) {
    return new Promise((resolve, reject) => {
      let contentType = ctObj;
      if (typeof contentType !== 'object') {
        reject(new Error('Content type object must be an object or an array'));
      }

      // add it to an array if it's not already an array
      if (!Array.isArray(contentType)) {
        contentType = [contentType];
      }
      resolve(merge(contentType));
    });
  }
