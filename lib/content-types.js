 ...
 * @param  {object} values - contains values for inputs
 * @param  {array} loadedTypes - array of merged content type objects
 * @param  {object} config - configuration object
 *
 * @return {promise} resolves with promise from `only` function
 */
const onlyCT = (type, values, loadedTypes, config) => {
  ...
  if (loadedTypes) {
    ...
    return only(ct, userConfig, config);
  }

  return merged('', config).then(types => {
