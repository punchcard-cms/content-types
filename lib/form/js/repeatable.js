...
(function formRepeatable() {
  ...
  var splitPop = function splitPop(input, splitter) {
      ...
      updateId = function updateId(id, index) {
        var splitId = id.split('--'),
            num = splitId[splitId.length - 1];
        if (!isNaN(num)) {
          if (index === undefined) {
            return splitPop(id, '--');
          }
          splitId[splitId.length - 1] = index;
      ...
      updateCurrentInstance = function updateCurrentInstance(instance, attribute) {
        ...
        if (index > allRepeatables[attribute].min) {
      ...
      initInstances = function initInstances(attribute) {
        var attr = document.querySelector('#' + attribute),
            container = attr.querySelector('.form--repeatable'),
            newContainer,
            idMap = {};

        // Checks if container has index
        if (container.id.split('--').length === 1) {
          updateCurrentInstance(container, attribute);

          // Gets updated values first instance
          container = attr.querySelector('.form--repeatable');
          while (allRepeatables[attribute].length < allRepeatables[attribute].min) {
            newContainer = container.cloneNode(true);

            // Updates each element in container
            newInstance(newContainer, idMap, allRepeatables[attribute].length);
            addNewContainer(container, newContainer);
            setIDsAndInputs(idMap, allRepeatables[attribute].length);
  ...
  window.onload = function onLoad() {
    Object.keys(allSettings).forEach(function repeatableHandler(input) {
      ...
      // Initializes instances to minimum value
      if (allRepeatables.hasOwnProperty(input) && allRepeatables[input].length <= allRepeatables[input].min) {
        initInstances(input);
      }

      // Checks if attribute has maximum instances initially
      if (allRepeatables.hasOwnProperty(input) && allRepeatables[input].length === allRepeatables[input].max) {
        attr = document.querySelector('#' + input);
        if (addButton) {
          attr.removeChild(addButton);
        }
      }
    });
  };
