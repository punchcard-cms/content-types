...
(function formRepeatable() {
  ...
  Object.keys(allSettings).forEach(function (input) {
      ...
      if (allRepeatables[input].length == 1) {
        ...
        childNodes.forEach(function(child) {
          if (child.id != "") {
            ...
            // htmlFor = child.htmlFor.split('--');
            // htmlFor.pop();
            // htmlFor.join('--');
            child.htmlFor = splitPop(child.htmlFor, '--');
          }
        });
        container.removeChild(container.querySelectorAll('#' + input + '-del')[0]);
      }
    };
    ...
    var addHandler = function (e) {
      ...
      var container = e.currentTarget.parentNode.querySelectorAll('div.form-repeatable')[0];

      // Modify name of initial instance and adds delete button
      if (allRepeatables[input].length == 1) {
        var newElement = document.createElement('input');
        newElement.type = 'button';
        newElement.value = 'delete';
        newElement.id = input + '-del';
      ...
      allRepeatables[input].length++;
      var containerSet = container.parentNode.querySelectorAll('div.form-repeatable');
    ...
    document.querySelectorAll('input.delete-button').forEach(function (button) {
