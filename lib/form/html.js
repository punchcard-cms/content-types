    ...
    let rendered = type.attributes.map(input => {
      ...
      if (Array.isArray(input.inputs)) {
        context.forEach((data, index) => {
          ...
          render += `<button type="button" class="delete-button" id="${input.id}-del--${index}">Delete</button>`;
          render += '</div>';
        });
        render += `<button type="button" class="add-button" id="${input.id}-add">Add</button>`;
      }
