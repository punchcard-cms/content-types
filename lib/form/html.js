...
const addRequired = (html, input) => {
  ...
  Object.keys(input.inputs).map(inp => {
    if (((input.hasOwnProperty('required')) && input.required === 'save') || ((input.inputs[inp].hasOwnProperty('required')) && input.inputs[inp].required === 'save')) {
      ...
      let find = new RegExp(`for=['"]\\s*${id}\\s*['"]`);
      render = render.replace(find, `for="${id}" class="${level}"`);

      // add required to input
      const name = `${input.id}--${inp}`;
      find = new RegExp(`name=['"]\\s*${name}\\s*['"]`);
      render = render.replace(find, `name="${name}" aria-required="true" required`);
    }
  });
  ...
  return new Promise((res) => {
    let rendered = type.attributes.map(input => {
      ...
      if (input.hasOwnProperty('required')) {
        required = `required required--${input.required}`;
      }
