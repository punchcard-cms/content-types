...
const rendered = (type) => {
  return new Promise((res, rej) => {
    ...
    // Checks if plugin directory has values
    if (!Array.isArray(dir)) {
      rej(new Error('Plugin directory congiguration must be an array'));
    }

    if (dir.indexOf(path.join(__dirname, '../../node_modules')) === -1) {
      dir.push(path.join(__dirname, '../../node_modules'));
    }
    ...
    for (let i = 0; i < type.attributes.length; i++) {
      if (type.attributes[i].hasOwnProperty('repeatable')) {
        allRepeatables[type.attributes[i].id] = _.cloneDeep(type.attributes[i].repeatable);
        allRepeatables[type.attributes[i].id].length = 1;
      }

      if (Array.isArray(type.attributes[i].inputs)) {
        type.attributes[i].inputs.forEach(input => {
          resolvedPlugins.push(_.cloneDeep(type.attributes[i]));
          resolvedPlugins[resolvedPlugins.length - 1].inputs = input;
        });
        allRepeatables[type.attributes[i].id].length = type.attributes[i].inputs.length;
      }
      else {
        resolvedPlugins.push(_.cloneDeep(type.attributes[i]));
      }
...
var allRepeatables = ${JSON.stringify(allRepeatables)};
// Work those requires
