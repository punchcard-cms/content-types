  ...
  return new Promise((res, rej) => {
    ...
    type.attributes.map(plugin => {
      ...
      children.forEach(input => {
        ...
        if (plugin.hasOwnProperty('required') && plugin.required === 'save') {
          inputs[inp.id].required = 'save';
        }

        // check if input is required, supersede main plugin
        if (inp.hasOwnProperty('required')) {
...
var required = function (input, settings, validation) {
  if (input.target.required && (input.target.required === 'save') && input.target.value === '') {
...
var validate = function (e) {
  ...
  // Run the validation
  validation = required(input, settings, allValidation[inp.parent.type][inp.validation.function]);
