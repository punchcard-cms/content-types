...
test('merged', t => {
  return types()
    .then(result => {
      ...
      t.is(result[0].key, 'something-new', 'Get first content type key');
      ...
      t.is(result[1].key, 'plugin-required-save', 'Get second content type key');
      ...
      t.is(result[2].key, 'new-text-thing', 'Get third content type key');
    });
});
